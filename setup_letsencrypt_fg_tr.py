...
def create_iam_policy(token, policy_name, region):
    actions = DNS_ACTIONS + ELB_ACTIONS + SCM_ACTIONS + CDN_ACTIONS
    body = {
        "role": {
            "display_name": policy_name,
            "description": "Let's Encrypt otomasyonu icin DNS + ELB + CCM + CDN yetkileri",
            "type": "AX",
            "policy": {
                "Version": "1.1",
                "Statement": [
    ...
    if "already" in resp.text.lower() or "duplicate" in resp.text.lower():
        # Policy mevcut - CCM (scm:*:*) ve CDN (cdn:*:*) yetkilerini eklemek icin guncelle
