# Huawei Cloud - Let's Encrypt SSL Certificates Automation
...
Huawei ELB, and updates the listeners. The certificate is also saved as a hosted certificate in
CCM (Cloud Certificate Manager) so it can be downloaded or used by other Huawei Cloud services.
The certificate is also deployed to CDN (Content Delivery Network) domains that match the
certificate domain list. No manual intervention is required.
...
- **CCM hosted certificate**: Certificate is also saved to CCM (Cloud Certificate Manager) for download or use by other services
- **CDN deployment**: Certificate is deployed to CDN domains matching the cert domain list (skipped if no match)
...
3. Create IAM custom policy (`dns:*:*` + `elb:*:*` + `scm:*:*` + `cdn:*:*`)
...
| `CCM_ENABLED` | `True` | Also save to CCM (Cloud Certificate Manager) as a hosted cert |
| `CCM_CERT_NAME` | `` | CCM cert name (empty = use CERT_NAME) |
| `CCM_REGION` | `ap-southeast-1` | CCM region (only cn-north-4, ap-southeast-1, my-kualalumpur-1) |
| `CDN_ENABLED` | `True` | Also deploy to CDN domains matching the cert domain list |
...
- **CCM**: Certificate is also saved to CCM (Cloud Certificate Manager) as a hosted cert.
  ...
  CCM is only available in `cn-north-4`, `ap-southeast-1`, `my-kualalumpur-1`.
  CCM errors are non-fatal (ELB cert still succeeds).
- **CDN**: Certificate is deployed to CDN domains matching the cert domain list.
  CDN domains must be created separately in the CDN console (requires origin server config).
  If no matching CDN domain exists, the CDN step is skipped (`action: "skip"`).
  CDN API is global (`cdn.myhuaweicloud.com`), available in `cn-north-1` and `ap-southeast-1`.
  CDN APIs require `enterprise_project_id=all` to see domains across all enterprise projects.
  CDN errors are non-fatal (ELB cert still succeeds).
- **Multi-value TXT records (DNS-01)**: When both `*.domain` and `domain` are in the SAN list,
  they share the same TXT record name. The new value is added to the existing recordset (via PUT)
  instead of deleting and recreating it, to avoid DNS cache issues with Let's Encrypt secondary validation.
