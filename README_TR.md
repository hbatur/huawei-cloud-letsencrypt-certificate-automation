# Huawei Cloud - Let's Encrypt SSL Sertifika Otomasyonu
...
Huawei ELB'ye yukler ve listener'lari gunceller. Sertifika ayrica CCM (Cloud Certificate Manager)
servisinde hosted certificate olarak kaydedilir, boylece download edilebilir veya baska servisler
tarafindan kullanilabilir. Sertifika ayrica sertifika domain listesiyle eslesen CDN (Content Delivery
Network) domain'lerine deploy edilir. Disaridan hicbir mudahale gerekmez.
...
- **CCM hosted certificate**: Sertifika ayrica CCM (Cloud Certificate Manager) servisine kaydedilir, download edilebilir veya baska servisler tarafindan kullanilabilir
- **CDN deployment**: Sertifika, cert domain listesiyle eslesen CDN domain'lerine deploy edilir (eslesme yoksa skip)
...
```
     ...
     (TXT kaydi) (ACME)  (cert)  (hosted (domain
                                     cert)  match)
```
...
3. IAM custom policy olustur (`dns:*:*` + `elb:*:*` + `scm:*:*` + `cdn:*:*`)
...
| `CCM_ENABLED` | `True` | CCM (Cloud Certificate Manager) hosted cert olarak kaydet |
| `CCM_CERT_NAME` | `` | CCM cert adi (bos = CERT_NAME kullan) |
| `CCM_REGION` | `ap-southeast-1` | CCM region (cn-north-4, ap-southeast-1, my-kualalumpur-1) |
| `CDN_ENABLED` | `True` | CDN domain'lerine sertifika deploy et |
| `CDN_CERT_NAME` | `` | CDN cert adi (bos = CERT_NAME kullan) |
...
- **CCM**: Sertifika ayrica CCM (Cloud Certificate Manager) servisine hosted cert olarak kaydedilir.
  CCM'in update API'si yoktur; yenileme sirasinda mevcut hosted cert silinip yeniden import edilir.
  CCM sadece `cn-north-4`, `ap-southeast-1`, `my-kualalumpur-1`? region'larinda mevcut.
  CCM hatalari non-fatal'dir (ELB cert yine de basarili olur).
- **CDN**: Sertifika, cert domain listesiyle eslesen CDN domain'lerine deploy edilir.
  CDN domain'leri ayrica CDN konsolundan olusturulmalidir (origin server config gerektirir).
  Eslesen CDN domain yoksa CDN adimi skip edilir (`action: "skip"`).
  CDN API global'dir (`cdn.myhuaweicloud.com`), `cn-north-1` ve `ap-southeast-1`'de mevcut.
  CDN API'leri `enterprise_project_id=all` parametresi gerektirir (tum enterprise project'lerdeki domain'leri gormek icin).
  CDN hatalari non-fatal'dir (ELB cert yine de basarili olur).
- **Multi-value TXT kayitlari (DNS-01)**: `*.domain` ve `domain` SAN listesindeyse,
  ayni TXT kayit adini paylasirlar. Yeni deger mevcut recordset'e eklenir (PUT ile),
  silinip yeniden olusturulmaz. Bu, Let's Encrypt secondary validation'da DNS cache sorunlarini onler.
...
- **Huawei Cloud**: FunctionGraph, DNS, ELB v3, IAM, CCM (SCM), CDN
